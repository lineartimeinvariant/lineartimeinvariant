...
offset = 0;
lastSample = 0;
downramp = (L-1:-1:0)/L; downramp = downramp(:);
...
while 1 % or while toc < N second
    ...
    if max(abs(x_emph)) > VADthres
        ...
        sigma_noise = sigma_noise/4; % arbitrary scaling
        %excit = zeros(L+P,1);
        ...
        if HNR > HNRthres
            ...
            if sw.useDiracDelta
                ...
                gKernelExt = repmat(gKernel,ceil(L/period),1);
                excit(offset+1:L) = gKernelExt(1:L-offset);
                % before ending this frame, pre-fill the first new offset samples 
                % of excit for next frame.
            end
        end
        excit = excit + sigma_noise * randn(L,1);
        if sw.debug, fprintf('\n');
        end
        ...
        for k = P+1:P+L
            ...
            slideWin = slideWin(:);
            thisFrameOut(k) = -A(2:end)*slideWin + excit(k-P);
        end
        lastSample = thisFrame(end);
        prevTail = thisFrameOut(L+1:L+P);

        offset = mod(offset - L, period);
        if HNR > HNRthres && offset > 0
            excit = [gKernel(period-offset+1:period); zeros(L-offset,1)]; % pre-filling for next frame.
        else
            excit = zeros(L,1);
        end
